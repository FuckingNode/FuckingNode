...
- Added `fav-editor` support for Zed, as well as the Flatpak versions of VSCode, VSCodium and Zed itself (differentiation is needed because `zed` and `dev.zed.Zed` aren't launched the same way from the command line).
...
- Added glob pattern support to `fkclean` destroyer.
